...
def test_provision_cognito_true_creates_user_and_row(
    client, cognito_client,
):
    """provision_cognito=true → AdminCreateUser called with
    the email + EMAIL delivery, AND the admin row inserted.
    Response reports cognito_provisioned=true."""
    r = client.post("/api/admin-roles", json={
...
def test_provision_cognito_false_skips_cognito(
    client, cognito_client,
):
    """Default (no flag) → row inserted, Cognito NOT called,
    even when the provider is cognito. The checkbox is
    opt-in per-add, not automatic."""
    r = client.post("/api/admin-roles", json={
        "email": "norcognito@test.com",
        "role":  "team_admin",
        "team_id": "t1",
    })
    assert r.status_code == 200
