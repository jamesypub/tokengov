...
[docs/saml-sso-setup.md](docs/saml-sso-setup.md). If your users reach
Bedrock **directly from an IAM
Identity Center permission-set role** (rather than assuming
`tg-consumer`), see "Govern a direct IDC permission-set role" in
[docs/idc-okta-setup.md](docs/idc-okta-setup.md) for the
`scripts/tg-idc-quota-permset.sh` reference an IDC admin runs to layer
the tg deny on durably.
