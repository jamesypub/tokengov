...
def regions(val: str) -> str | None:
    """A comma-separated region list (the Bedrock invocation-logging
    capture regions). Empty is valid — it means "skip logging". Each
    non-empty entry must be a real AWS region. Region-config is a
    per-region singleton, so duplicates are pointless but harmless
    (the installer de-dupes)."""
    v = (val or "").strip()
    if v == "":
        return None
    for part in v.split(","):
        ...
        if part == "":
            continue
        # Region tokens may carry a 2-digit trailing number
        # (us-east-1, ap-southeast-1) — broader than the single-region
        # validator's \d, matching every real AWS region.
        if not re.match(r"^[a-z]{2}-[a-z]+-\d+$", part):
            return f"'{part}' is not an AWS region (e.g. us-east-1)"
    return None
