...
from worker.principal_classify import (
    resolve_principal, classify_role_type,
)
...
def run() -> dict:
    ...
    with get_db() as db:
        # Admin-maintained email↔Bedrock-key map, built once per batch:
        # {iam_user_name -> (identity_key, email)} for every user who
        # recorded a key. resolve_principal() uses it to re-attribute a
        # `user/<name>` CUR principal (a long-term Bedrock API key,
        # backed by an IAM user) to the mapped developer, so mantle /
        # Codex bearer-key spend lands on the person in cur_user_spend.
        # Empty when no admin has mapped a key → resolve_principal is a
        # no-op and classification is unchanged.
        key_map: dict[str, tuple[str, str]] = {
            name: (ik or em, em)
            for (name, ik, em) in db.query(
                User.bedrock_key_user, User.identity_key, User.email
        ...
        for row in rows:
            ...
            # resolve_principal = classify_principal + key re-attribution
            # (a mapped `user/<name>` key lands on the owning developer;
            # everything else is classified exactly as before).
            identity_key, email, ptype, principal_arn = (
                resolve_principal(arn, key_map))
            if not identity_key:
                continue
