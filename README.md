...
It works because every Bedrock invocation under SSO carries the
caller's email in the IAM session ARN, so AWS CUR 2.0 attributes the
...
whatever SSO role the customer already has."

---

## Architecture

SSO sessions assume one scoped IAM role; spend is attributed
per user from AWS CUR 2.0 (the sole spend source); the reconciler
...
- [docs/idc-okta-setup.md](docs/idc-okta-setup.md) — wiring AWS IDC to Okta
- [docs/saml-sso-setup.md](docs/saml-sso-setup.md) — configure company SSO (SAML via IDC) at runtime from Settings
