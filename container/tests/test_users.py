...
def test_managed_role_env_var_override(client, monkeypatch):
    """#345: TG_MANAGED_ROLE_NAMES overrides the default
    list. A reload of the users module recomputes the
    classification set."""
    monkeypatch.setenv(
        "TG_MANAGED_ROLE_NAMES", "AcmeBedrock,foo")
    import importlib, sys
    sys.modules.pop("api.routes.users", None)
    import api.routes.users as ur
    importlib.reload(ur)
    # Use the reloaded helper directly — exercising the
    # full FastAPI route would require re-mounting the
    # reloaded router.
    from db.models import User
    u = User(
        email="dev@test.com",
