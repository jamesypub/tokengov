...
- the **IdP metadata URL** — the address where your identity
  provider publishes its SAML configuration (issuer, certificates,
  endpoints); your IdP admin has it. Preferred over an uploaded
  metadata XML because Cognito auto-refreshes it,
...
Run the command that matches how you installed — local, ECS, or the
optional CUR add-on.
