...
own AWS identity** (in this install, an SSO session) — no new keys,
...
**BYO-VPC subnet requirements.** Pick ONE egress pattern for all the
subnets you pass — mixing them breaks task startup (details below).
The Fargate task pulls its
...
authoritative dollar figure on your AWS bill.** What this means for
you: caps enforce on **billed** spend, which lags the AWS bill by
**≤24h** — not a real-time estimate. There is no estimate-from-logs
layer; CUR is the sole spend source.
