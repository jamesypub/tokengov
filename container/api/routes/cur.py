...
def cur_health(
    ...
    """#726: classify the CUR spend source. org-admin scoped.
    Returns {status, detail} where status is one of absent /
    columns_missing / principal_blank / principal_absent_from_export
    / healthy. #749: the last two split the old single
    'principal_blank' state into present-but-empty (wait for the next
    delivery) vs absent-from-the-delivered-Parquet (re-create the
    export) using the S3 manifest column-set."""
