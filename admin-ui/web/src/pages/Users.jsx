...
import { isExternalIdp, ssoName, helpText, inviteText } from '../lib/inviteCopy'
...
// IDC is a QUALIFIER on top of governed/ungoverned, not its own
// terminal state. The ◆ badges an Identity Center user. For a governed
// IDC user, governing records the limit but it is only active once an
// identity administrator applies the governance policy — a state
// confirmed on the user's own page (the list doesn't per-row verify).
// So the qualifier must NOT imply "enforced" here.
//
// UI-copy directive (owner): plain business language, no internal
// resource/role/policy names, no mechanism internals. The legend
// points the admin to the user page for the exact status.
export const IDC_QUALIFIER = {
  icon: '◆',
  label: 'IDC',
  desc: 'Identity Center user. Governing records the limit, but it is '
    + 'active only after an identity administrator applies the '
    + 'governance policy to their access — open the user to see whether '
    + 'it’s enforced or still pending.',
  cls: 'text-amber-600',
}
...
const _DRIFT_COPY = {
  ...
  // The remedy differs by cause: for a non-IDC principal the attach
  // failed/lagged and can be re-applied in place; for an IDC
  // (Identity Center) principal tg CAN'T attach the deny to the
  // permission-set role — enforcement is pending on the identity side
  // (an identity admin must apply the governance policy). So we branch
  // the action by isIdc, consistent with the honest UserDetail badge.
  governed_no_deny: {
    plain: who =>
      `${who} is set to be governed, but the spend and model `
      + `limits aren’t actually being enforced yet.`,
    action: who =>
      `Re-apply now to attach enforcement (or open ${who} and `
      + `Ungovern if they shouldn’t be governed).`,
    // IDC: re-applying Govern can't attach it — enforcement is
    // pending on the identity provider. Plain language, no internal
    // names (matches the shipped IDC badge).
    actionIdc: who =>
      `Not yet active — because ${who} signs in through your identity `
      + `provider, an identity administrator must apply the governance `
      + `policy to their access before limits take effect.`,
  },
...
export function driftBannerModel(drift, sweepAt) {
  ...
  return {
    ...
    sweepLabel: sweepAt
      ...
      : null,
    // The job that produces this banner. Naming it here lets the admin
    // connect the banner to the same-named "Run now" entry on the Jobs
    // page — where the on-demand re-check lives (the banner links there
    // rather than triggering the sweep itself).
    jobName: 'Governance drift check',
    rows: list.map(d => {
      const who = d.email || d.identity_key || '(unknown principal)'
      const copy = _DRIFT_COPY[d.direction]
      const navId = d.identity_key || d.email
      const rowIsIdc = (d.role_type || 'iam') === 'idc'
      // "Re-apply now" only helps the non-IDC governed_no_deny case:
      // it re-runs the synchronous reconcile to attach the deny. For
      // IDC that attach is impossible (identity-side), and for
      // deny_no_governed the fix is Ungovern (not a re-apply).
      const canReapply = d.direction === 'governed_no_deny' && !rowIsIdc
      return {
...
// The governance-drift banner. Informational + navigational, plus a
// per-row "Re-apply now" for the non-IDC governed_no_deny case that
// runs the shared synchronous reconcile (reconcile_principal) and
// reflects the real result — no page hop, no ~5-min wait. IDC drift
// rows get the honest "pending on the identity side" guidance instead
// (re-applying Govern can't attach an AWSReservedSSO_* role). Never
// auto-sets `governed`.
export function DriftBanner({ drift, sweepAt, onReapplied }) {
  // Per-row state keyed by identityKey: {busy, result} where result is
  // the honest apply outcome (or an error message).
  const [state, setState] = React.useState({})
  const b = driftBannerModel(drift, sweepAt)
  if (!b.show) return null

  async function reapply(row) {
    const k = row.identityKey
    setState(s => ({ ...s, [k]: { busy: true } }))
    try {
      const res = await api.reapplyGovernance(k)
      const a = res && res.apply ? res.apply : res
      setState(s => ({ ...s, [k]: { busy: false, result: a } }))
      // Refresh the drift list so a now-enforced row clears.
      if (typeof onReapplied === 'function') onReapplied()
    } catch (e) {
      setState(s => ({
        ...s, [k]: { busy: false, result: { state: 'failed', detail: e.message } },
      }))
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="mb-3 text-[13px]">
        Checked by <span className="font-semibold">{b.jobName}</span>
        {b.sweepLabel ? (
          <> — last ran {b.sweepLabel}. To re-check right now, run{' '}
          <span className="font-semibold">{b.jobName}</span> on the{' '}
          <a href="#/jobs" className="underline hover:no-underline font-medium">
            Jobs page →
          </a></>
        ) : (
          <> — hasn’t run yet. Run{' '}
      ...
      <ul className="space-y-2">
        {b.rows.map((r, i) => {
          ...
          return (
            ...
            >
              {r.href ? (
                <a href={r.href} className="font-medium underline hover:no-underline">
                  {r.who}
                </a>
              ) : (
              ...
              {/* Non-IDC governed_no_deny → one-click re-apply (the
                  shared synchronous reconcile). IDC rows never show it
                  (it can't attach their role). */}
              {r.canReapply && (
                <div className="mt-2 flex items-center gap-2">
                  <button
                    ...
                    disabled={st.busy}
                    onClick={() => reapply(r)}
                    className="text-[12px] rounded border border-amber-400 px-2 py-0.5 bg-white hover:bg-amber-100 disabled:opacity-50"
                  >
                    {st.busy ? 'Applying…' : 'Re-apply now'}
                  </button>
                  {res && (
                    <span role="status" aria-live="polite" className="text-[12px]">
                      {res.enforced
                        ? '✓ enforced'
                        : res.state === 'failed'
                          ? `⚠ not applied — ${res.detail || 'retry'}`
                          : res.state === 'pending_idc'
                            ? 'pending (identity side)'
                            : 'applied — confirming'}
                    </span>
                  )}
                </div>
              )}
              {/* raw internals behind a disclosure. */}
...
export default function Users() {
  ...
  useEffect(() => { loadDrift() }, [isOrgAdmin])
  ...
  const columns = useMemo(() => [
    ...
    {
      // optional low-prominence Bedrock-key column (the IAM-user
      // name of the user's mantle/Codex API key). A power-admin field —
      // em-dash when unmapped; kept near the end, hidden on narrow
      // widths so it never crowds the primary columns.
      id: 'bedrock_key',
      accessorKey: 'bedrock_key_user',
      header: 'Bedrock key',
      enableSorting: false,
      ...
      cell: info => {
        ...
        return v
          ? <span className="font-mono text-[12px] text-[var(--ink-3)]">{v}</span>
          : <em className="text-[var(--ink-4)]">—</em>
      },
    },
    {
      id: 'chevron',
  ...
  return (
    <div className="p-8">
      <header className="flex justify-between items-center mb-4">
        ...
        <Button variant="primary" onClick={() => setShowPrereg(true)}>+ Add user</Button>
      </header>
      ...
      <DriftBanner drift={drift} sweepAt={driftSweepAt} onReapplied={loadDrift} />
      ...
      {showPrereg && <AddUserModal onClose={() => { setShowPrereg(false); load() }} />}
    </div>
  )
}
...
function AddUserModal({ onClose }) {
  ...
  const [role, setRole] = useState('member')   // least-privilege default
  const [teamId, setTeamId] = useState('')
  const [teams, setTeams] = useState([])
  const [providers, setProviders] = useState(null)
  const [err, setErr] = useState(null)
  const [busy, setBusy] = useState(false)
  // Hold the modal open on a done screen with a copyable invite (the
  // SSO branch gets no Cognito email; the invite is the manual channel).
  ...
  // Team is required ONLY for team_admin — a team_admin with no team
  // administers nothing. Org Admin / Member need no team.
  const teamRequired = role === 'team_admin'
  ...
  async function submit(e) {
    ...
    if (teamRequired && !teamId) {
      setErr('A Team Admin must be assigned a team'); return
    }
    setBusy(true); setErr(null)
    try {
      // Route through grant_role: it authorizes and (whenever tg owns
      // the directory) auto-provisions the Cognito login — no checkbox.
      // team_id is sent only when the role needs one.
      await api.grantRole({
        email: email.trim().toLowerCase(),
        role,
        team_id: teamRequired ? teamId : null,
      })
      setDone(true)
    } catch (e) {
      setErr(e.message)
  ...
  async function copyInvite() {
    const text = inviteText(providers, origin)
    try {
      await navigator.clipboard.writeText(text)
    } catch {
      // Clipboard API unavailable (insecure context / permissions) —
      // still flip the state so the aria-live announces and the user
      // can select the visible text as a fallback.
    }
    setCopied(true)
  }

  if (done) {
    const external = isExternalIdp(providers)
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl p-6 max-w-md w-full flex flex-col gap-3 shadow-xl">
          ...
          >
            {external ? (
              <div>
                {email.trim()} can now be recognized by tg. Before they
                can sign in, grant them access to the tg application in{' '}
                {ssoName(providers)}. Then send them the invitation below.
              </div>
            ) : (
              <div>
                tg created {email.trim()}’s login and emailed them an
                invitation. You can also send them the message below.
              </div>
            )}
            <div className="mt-2 text-[var(--ink-4)] whitespace-pre-wrap">
              {inviteText(providers, origin)}
            </div>
          </div>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <form onSubmit={submit} className="bg-white rounded-xl p-6 max-w-md w-full flex flex-col gap-3 shadow-xl">
        <div className="flex justify-between items-center">
          <h3 className="m-0 text-lg font-semibold">Add user</h3>
          <button type="button" onClick={onClose} className="text-xl text-[var(--ink-4)] hover:text-[var(--ink)]">✕</button>
        </div>
        <p className="m-0 text-sm text-[var(--ink-4)]">
          Give someone access to Token Governance. Enter their work
          email and choose what they can do.
        </p>
        ...
        <div className="mt-2 flex justify-end gap-2">
          ...
          <Button type="submit" variant="primary" disabled={busy}>{busy ? 'Submitting…' : 'Add user'}</Button>
        </div>
      </form>
    </div>
  )
