...
def build_version() -> str:
    ...
    try:
        ...
        # Dirty iff an IMAGE-SOURCE file changed vs HEAD — scope to the
        # paths the Docker build COPYs (container, admin-ui/web). Bare
        # `git status --porcelain` counted untracked scratch files (e.g.
        # internal/researcher2/) and flipped -dirty on a pristine tree.
        st = subprocess.run(
            ["git", "status", "--porcelain", "--", "container", "admin-ui/web"],
            cwd=str(REPO_ROOT), capture_output=True, text=True, timeout=5)
