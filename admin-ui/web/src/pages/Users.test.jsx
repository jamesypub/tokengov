...
const reapplyGovernance = vi.fn()
vi.mock('../api', () => ({
  api: { reapplyGovernance: (...a) => reapplyGovernance(...a) },
}))
...
import {
  ...
  headerAlignClasses, SummaryTile, DriftBanner,
} from './Users'
...
describe('driftBannerModel (#818/#846)', () => {
  ...
  // The banner names the job that produced it so the admin can find
  // the same-named "Run now" entry on the Jobs page (where the
  // on-demand re-check lives).
  it('exposes the job name (Governance drift check)', () => {
    expect(driftBannerModel(sample, '2026-06-08T19:30:00+00:00').jobName)
      .toBe('Governance drift check')
    // job name is present even when the sweep has never run
    expect(driftBannerModel(sample, null).jobName)
      .toBe('Governance drift check')
  })
  ...
  it('governed_no_deny (non-IDC) → plain + Re-apply action + canReapply', () => {
    ...
    // non-IDC → the one-click re-apply affordance
    expect(r.isIdc).toBe(false)
    expect(r.canReapply).toBe(true)
    expect(r.action).toMatch(/Re-apply now/i)
    expect(r.plain).not.toMatch(/tg-BedrockQuotaDeny|governed=true|deny-not-attached/)
  })

  it('governed_no_deny (IDC) → pending-on-identity-side, NO re-apply', () => {
    const r = driftBannerModel([{
      identity_key: 'idc@corp.com', email: 'idc@corp.com',
      role_arn: 'arn:aws:iam::1:role/aws-reserved/sso.amazonaws.com/'
        + 'AWSReservedSSO_Dev_abc',
      role_type: 'idc',
      direction: 'governed_no_deny',
      expected: 'governed', actual: 'deny-not-attached',
    }], null).rows[0]
    expect(r.isIdc).toBe(true)
    expect(r.canReapply).toBe(false)          // can't help an IDC row
    // honest identity-side language, not "re-apply Govern"
    expect(r.action).toMatch(/identity administrator/i)
    expect(r.action).not.toMatch(/Re-apply now/i)
    // no internal names in the primary copy
    expect(r.action).not.toMatch(/tg-|AWSReservedSSO|permission-set policy/)
  })

  it('role_type from the API row drives isIdc (defaults iam)', () => {
    const iam = driftBannerModel(sample, null).rows[0]
    expect(iam.isIdc).toBe(false)   // sample has no role_type → iam
  })
...
// The DriftBanner render — non-IDC governed_no_deny gets a
// working "Re-apply now" (calls the shared reconcile + shows the honest
// result); an IDC row shows pending-guidance and NO re-apply button.
describe('DriftBanner re-apply', () => {
  beforeEach(() => reapplyGovernance.mockReset())

  const nonIdcRow = {
    identity_key: 'dev@corp.com', email: 'dev@corp.com',
    role_arn: 'arn:aws:iam::1:role/AppRole', role_type: 'iam',
    direction: 'governed_no_deny', expected: 'governed', actual: 'drift',
  }
  const idcRow = {
    identity_key: 'idc@corp.com', email: 'idc@corp.com',
    role_arn: 'arn:aws:iam::1:role/aws-reserved/sso.amazonaws.com/'
  ...
  it('non-IDC row shows Re-apply now; click calls the shared reconcile', async () => {
    reapplyGovernance.mockResolvedValue({
      identity_key: 'dev@corp.com',
      apply: { state: 'enforced', enforced: true, denied: true },
    })
    const onReapplied = vi.fn()
    render(<DriftBanner drift={[nonIdcRow]} sweepAt={null} onReapplied={onReapplied} />)
    const btn = screen.getByRole('button', { name: /Re-apply now/i })
    fireEvent.click(btn)
    await waitFor(() => expect(reapplyGovernance).toHaveBeenCalledWith('dev@corp.com'))
    // the status span specifically (role=status), not the plain
    // "…not being enforced yet" copy which also contains "enforced"
    expect(await screen.findByRole('status')).toHaveTextContent(/✓ enforced/)
    expect(onReapplied).toHaveBeenCalled()
  })

  it('IDC row shows pending guidance and NO Re-apply button', () => {
    render(<DriftBanner drift={[idcRow]} sweepAt={null} onReapplied={() => {}} />)
    expect(screen.queryByRole('button', { name: /Re-apply now/i })).toBeNull()
    expect(screen.getByText(/identity administrator/i)).toBeInTheDocument()
  })

  it('re-apply failure shows the real error, not a false success', async () => {
    reapplyGovernance.mockResolvedValue({
      apply: { state: 'failed', enforced: false, detail: 'throttled' },
    })
    render(<DriftBanner drift={[nonIdcRow]} sweepAt={null} onReapplied={() => {}} />)
    fireEvent.click(screen.getByRole('button', { name: /Re-apply now/i }))
    expect(await screen.findByText(/not applied/i)).toBeInTheDocument()
  })

  // The banner names the job + its last-run and points the admin
  // to the Jobs page to re-check now (it does NOT trigger the sweep
  // itself — the run-now trigger lives on the Jobs page).
  it('names the job + last-run and links to the Jobs page', () => {
    render(
      <DriftBanner
        drift={[nonIdcRow]}
        sweepAt={'2026-07-12T04:30:00+00:00'}
        onReapplied={() => {}}
      />)
    // job identity + last-run time
    expect(screen.getAllByText(/Governance drift check/).length)
      .toBeGreaterThan(0)
    expect(screen.getByText(/last ran 2026-07-12 04:30 UTC/))
      .toBeInTheDocument()
    // a real, keyboard-reachable link to the Jobs page (hash route)
    const link = screen.getByRole('link', { name: /Jobs page/i })
    expect(link.getAttribute('href')).toBe('#/jobs')
  })

  it('never-run sweep → banner still names the job + links to Jobs', () => {
    render(<DriftBanner drift={[nonIdcRow]} sweepAt={null} onReapplied={() => {}} />)
...
// the optional "Bedrock key" column. Defined inside the page
// body's useMemo (not an exported cell), so pin it via source: the
// column exists on bedrock_key_user, renders the value or an em-dash
// when null, and is non-sorting / hidden-on-narrow (a power-admin
// field that never crowds the primary columns).
import { readFileSync } from 'fs'

describe('Users list — Bedrock key column', () => {
  const src = readFileSync('src/pages/Users.jsx', 'utf8')

  it('adds a non-sorting Bedrock key column keyed on bedrock_key_user', () => {
    expect(src).toMatch(/id: 'bedrock_key'/)
    expect(src).toMatch(/accessorKey: 'bedrock_key_user'/)
    expect(src).toMatch(/header: 'Bedrock key'/)
    expect(src).toMatch(/enableSorting: false/)
  })

  it('renders the value when set and an em-dash when null', () => {
    // the cell branches on the value: <span>{v}</span> : <em>—</em>
    expect(src).toMatch(/const v = info\.getValue\(\)/)
