...
def _scripted(**overrides):
    base = {
        ...
        "vpc_mode": VPC_CREATE,
        # Invocation-logging capture (slice 3): base flow defaults to
        # Skip so existing full-flow to_env assertions are unchanged
        # (no TG_INVLOGS_REGIONS); the dedicated invlogs tests override.
        "invlogs_choice": INVLOGS_SKIP,
    }
...
# ── dirty-check is PATH-SCOPED to image sources ────────────────────────
# The version stamp derives -dirty from an IMAGE-SOURCE tracked-diff
# (`git status --porcelain -- container admin-ui/web`), NOT a bare
# porcelain that counts untracked scratch files. A pristine tree with
# untracked internal/researcher2/ drafts must NOT stamp -dirty; a real
# edit under container/ or admin-ui/web/ still must.

def _fake_git_pathscoped(sha="abc1234", scoped_dirty=False):
    """subprocess.run replacement that HONORS the pathspec: `status
    --porcelain -- <paths>` returns non-empty only when scoped_dirty is
    set AND the invocation actually carries a `--` pathspec. A bare
    `git status --porcelain` (no pathspec) is treated as the old
    untracked-counting behavior and returns a scratch-file entry — so a
    regression that drops the scope would FLIP this test to -dirty."""
    def _run(cmd, *a, **k):
        if "rev-parse" in cmd:
            return _GitProc(0, sha)
        if "status" in cmd:
            has_scope = "--" in cmd and "container" in cmd
            if not has_scope:
                # Unscoped: mimic untracked scratch files being counted.
                return _GitProc(0, "?? internal/researcher2/draft.md\n")
            return _GitProc(0, ("M container/api/app.py\n"
                                if scoped_dirty else ""))
        return _GitProc(0, "")
    return _run


def test_build_version_clean_with_untracked_scratch(monkeypatch, tmp_path):
    # Pristine image sources, but untracked scratch files exist: the
    # path-scoped check sees no image-source change → NO -dirty. (The
    # false-positive this fixes: ~196 untracked internal/researcher2/
    # files flipped -dirty on a `reset --hard origin/main` tree.)
    import tg_cli.runner as runner
    (tmp_path / "VERSION").write_text("1.1.1\n")
    monkeypatch.setattr(runner, "REPO_ROOT", tmp_path)
    monkeypatch.setattr(runner.subprocess, "run",
                        _fake_git_pathscoped(sha="72a497b"))
    assert runner.build_version() == "v1.1.1-g72a497b"


def test_build_version_dirty_on_image_source_edit(monkeypatch, tmp_path):
    # A real tracked edit under container/ (an image source) DOES stamp
    # -dirty — the fix stays honest.
    import tg_cli.runner as runner
    (tmp_path / "VERSION").write_text("1.1.1\n")
    monkeypatch.setattr(runner, "REPO_ROOT", tmp_path)
    monkeypatch.setattr(runner.subprocess, "run",
                        _fake_git_pathscoped(sha="72a497b",
                                             scoped_dirty=True))
    assert runner.build_version() == "v1.1.1-g72a497b-dirty"


def test_build_version_status_call_is_path_scoped():
    # Guard the scope in the source: the status invocation must carry the
    # image-source pathspec, so a refactor can't silently revert to the
    # untracked-counting bare porcelain.
    import inspect
    import tg_cli.runner as runner
    src = inspect.getsource(runner.build_version)
    assert '"container"' in src and '"admin-ui/web"' in src
    assert '"--"' in src
...
# ── invocation-logging capture regions (slice 3) ─────────────────────

@pytest.mark.parametrize("val,ok", [
    ("", True),                                   # blank = skip
    ("us-east-1", True),
    ("us-east-1, eu-west-2, ap-southeast-1", True),
    ("nonsense", False),
    ("us-east-1, bogus", False),
])
def test_regions_validator(val, ok):
    assert (V.regions(val) is None) == ok


def test_invlogs_noninteractive_defaults_on_to_install_region():
    # --non-interactive with no supplied value → capture ON in the
    # install region (default-on, per the AC).
    r = Resolver(interactive=False, supplied={})
    a = _ask_invlogs(r, {"region": "us-east-1"})
    assert a["invlogs_regions"] == "us-east-1"


def test_invlogs_env_list_honored_noninteractive():
    r = Resolver(interactive=False,
                 supplied={"invlogs_regions": "us-east-1,eu-west-2"})
    a = _ask_invlogs(r, {"region": "us-east-1"})
    assert a["invlogs_regions"] == "us-east-1,eu-west-2"


def test_invlogs_choice_skip():
    r = Resolver(interactive=True,
                 scripted={"invlogs_choice": INVLOGS_SKIP})
    a = _ask_invlogs(r, {"region": "us-east-1"})
    assert a["invlogs_regions"] == ""


def test_invlogs_choice_here_uses_install_region():
    r = Resolver(interactive=True,
                 scripted={"invlogs_choice": INVLOGS_ON})
    a = _ask_invlogs(r, {"region": "eu-west-2"})
    assert a["invlogs_regions"] == "eu-west-2"


def test_invlogs_choice_custom_multiregion():
    r = Resolver(interactive=True, scripted={
        "invlogs_choice": INVLOGS_CUSTOM,
        "invlogs_regions": "us-east-1,eu-west-2"})
    a = _ask_invlogs(r, {"region": "us-east-1"})
    assert a["invlogs_regions"] == "us-east-1,eu-west-2"


def test_to_env_maps_invlogs_when_set():
    env = to_env({"region": "us-east-1", "bootstrap_email": "a@b.com",
                  "invlogs_regions": "us-east-1,eu-west-2"})
    assert env["TG_INVLOGS_REGIONS"] == "us-east-1,eu-west-2"


def test_to_env_omits_invlogs_when_skipped():
    env = to_env({"region": "us-east-1", "bootstrap_email": "a@b.com",
                  "invlogs_regions": ""})
    assert "TG_INVLOGS_REGIONS" not in env
