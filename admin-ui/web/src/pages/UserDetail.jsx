...
import {
  ...
  IDC_GOVERN_NOTICE_TITLE, IDC_GOVERN_NOTICE_BODY, IDC_GOVERNED_NOTE,
} from '../lib/governGate'
...
function AccessBadge({ user }) {
  const t = user.principal_type
  // #1011: IDC is governable now. A governed IDC user reads
  // "✓ Governed (IDC)"; an ungoverned one shows the ◆ IDC badge with
  // the advisory precondition. The badge is distinct from an IAM-role
  // governed user so the advisory-enforcement nuance stays visible.
  if (isIdc(user)) {
    if (user.governed) {
      return (
        <div>
    ...
    <div className="p-8">
            ...
            <>
              ...
              {((actions.includes('force_block') && canPerform('force_block', tier))
                ...
                <div className="flex gap-2 flex-wrap items-start mt-3 pt-3 border-t border-[var(--border-2)]">
                  {actions.includes('unblock') && canPerform('unblock', tier) && (
                    <Button variant="primary" disabled={busy}
                      onClick={() => act(() => api.unblock(email, user.version), 'Unblocked — cap governance restored')}>
                      Unblock
                    </Button>
                  )}
                  {actions.includes('force_block') && canPerform('force_block', tier) && (
      ...
      <TypedConfirmModal
        ...
        onConfirm={() => {
          ...
          act(() => api.forceBlock(email, email, user.version), 'Force blocked')
        }}
      />
        ...
        <ManageModal
          ...
          onConfirm={() => {
            ...
            act(
              ...
              'Now governed. Enforcement (spend cap + any blocked '
              + 'models) takes effect within a few minutes, on the '
              + 'next governance job run.',
            )
          }}
        />
      )}
...
// want them.
export function GovernancePanel({ user }) {
  const g = governanceState(user)
            ...
            {g === 'governed' ? (
              <>
                ...
                {/* plain-language self-detach advisory (#837); for IDC
                    the analogous caveat is the advisory permission-set
                    path, stated in IDC_GOVERNED_NOTE above. */}
                {!isIdc(user) && (
...
function EnableLoginModal({ user, onClose, onDone }) {
  ...
  // tg owns the directory when Cognito provisioning is on (#926 flag,
  // surfaced via /auth/providers). Until loaded, assume Cognito (the
  // default) so the copy isn't misleading on the common path.
  const tgOwns = providers ? !!providers.cognito_provisioning : true
  const ssoName = providers?.okta_display_name || 'your SSO'
  ...
  async function submit() {
    ...
    try {
      ...
      const msg = res.cognito_provisioned
        ? `Login enabled for ${who} — invite emailed. They click `
          + `"Forgot password" to set their own password.`
        : `Login enabled for ${who} — they sign in via ${ssoName} `
          + `(no email sent).`
      onDone(msg)
  ...
  return (
    <ModalShell onClose={onClose}>
      ...
      <p className="text-sm text-[var(--ink-3)] m-0 mb-3">
        {tgOwns
          ? <>tg will create their login and email <span
              className="font-mono text-[12px]">{user.email}</span>; they
              click <strong>Forgot password</strong> on the sign-in page
              to set their own password.</>
          : <>This deployment federates to <strong>{ssoName}</strong> —
              tg will authorize <span className="font-mono text-[12px]">
              {user.email}</span> but send no email; they sign in via
              {' '}{ssoName}.</>}
      </p>
      <label className="block text-sm font-medium mb-1">Role</label>
      <select className="w-full border rounded px-2 py-1 mb-1 text-sm"
      ...
      <p className="text-xs text-[var(--ink-3)] m-0 mb-2">
