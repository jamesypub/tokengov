...
def grant_role(
    ...
    # #357: optionally also create the user in the Cognito
    # pool (one-click "Send invite"). Ignored unless tg owns the
    # directory (#926 tg_owns_directory) — on an external-IdP
    # deployment a stray flag is a no-op, never a 500, so the same
    # client code works against any provider.
    provision_cognito = bool(body.get("provision_cognito"))
    ...
    if provision_cognito and _cognito_provisioning_enabled(db):
