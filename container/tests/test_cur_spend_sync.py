...
# ── Layer-1: re-attribute a mapped Bedrock-key principal ──

def test_sync_reattributes_mapped_bedrock_key_to_owner(
    clean_db, monkeypatch,
):
    """A `user/<name>` key principal (a long-term Bedrock API key,
    backed by an IAM user) whose name an admin mapped to a developer
    lands the spend under THAT developer in cur_user_spend — the
    mantle/Codex attribution fix."""
    from db.models import User as U
    with get_db() as db:
        db.add(U(email="dev@corp.com", identity_key="dev@corp.com",
                 status="active",
                 bedrock_key_user="MantleApiKey-uhbhn79a"))
    model = "us.anthropic.claude-sonnet-4-6"
    rows = [
        _r("arn:aws:iam::123456789012:user/MantleApiKey-uhbhn79a",
           _HOUR, "us-east-1", model, "7.00", "700"),
    ]
    job = _stub(monkeypatch, rows)
    out = job.run()
    assert out["status"] == "ok"
    with get_db() as db:
        # spend landed under the mapped developer, NOT the raw key name
        row = db.query(CurUserSpend).filter(
            CurUserSpend.email == "dev@corp.com").one()
        assert row.spend_usd == 7.00
        assert db.query(CurUserSpend).filter(
            CurUserSpend.email == "MantleApiKey-uhbhn79a").count() == 0


def test_sync_unmapped_key_stays_raw(clean_db, monkeypatch):
    """Regression: a key IAM-user with NO mapping keeps its raw
    iam_user principal (keyed on the name) — unchanged behavior."""
    model = "us.anthropic.claude-sonnet-4-6"
    rows = [
        _r("arn:aws:iam::123456789012:user/BedrockAPIKey-toqd",
           _HOUR, "us-east-1", model, "3.00", "300"),
    ]
    job = _stub(monkeypatch, rows)
    job.run()
    with get_db() as db:
        row = db.query(CurUserSpend).filter(
            CurUserSpend.email == "BedrockAPIKey-toqd").one()
        assert row.spend_usd == 3.00
