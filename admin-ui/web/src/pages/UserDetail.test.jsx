...
import {
  ...
  showOverCapAffordance, BedrockKeySection,
} from './UserDetail'
...
describe('GovernancePanel plain-language status (#837)', () => {
  ...
  it('#1011: ungoverned IDC shows the governable advisory, not "not governable"', () => {
    ...
    // precondition, in plain business language (per the owner's
    // UI-copy directive) — no terminal "not governable" copy, no
    // internal tg-* names. Raw ARN jargon (AWSReservedSSO_*) is allowed
    // only inside the Technical details disclosure, not the primary copy.
    expect(screen.getByText(/Ungoverned/i)).toBeInTheDocument()
  ...
  it('governed IDC with UNVERIFIED enforcement reads "pending", not "enforced"', () => {
    // No enforcement prop (or enforced:false) = the honest default:
    // governed intent set, NOT yet enforced. demo0's real state.
    const { container } = render(<GovernancePanel user={{
      role_type: 'idc', governed: true, email: 'dev@corp.com',
      principal_arn: 'arn:aws:sts::1:assumed-role/AWSReservedSSO_x/dev@corp.com',
    }} enforcement={{ state: 'pending', enforced: false }} />)
    // amber "pending enforcement" badge, NOT the green success state
    expect(container.textContent).toMatch(/pending enforcement/i)
    // the false "enforced" claim is gone from the pending path
    expect(container.textContent).not.toMatch(/enforced via/i)
    expect(container.textContent).not.toMatch(/is enforcing this principal/i)
    // honest plain-language pending note + the finish-enabling disclosure
    expect(container.textContent).toMatch(/not yet active/i)
    expect(container.textContent).toMatch(/How to finish enabling this/i)
    // UI-copy directive: no internal tg-* names in the primary copy
    // (they may live only inside the disclosure — assert none anywhere
    // here, since our copy uses none at all)
    expect(container.textContent).not.toMatch(/tg-BedrockQuotaDeny|tg-consumer|tg-QuotaDenyPermissionSet|tg-idc-quota-permset/)
  })

  it('governed IDC with VERIFIED enforcement MAY read "enforced"', () => {
    const { container } = render(<GovernancePanel user={{
      role_type: 'idc', governed: true, email: 'dev@corp.com',
      principal_arn: 'arn:aws:sts::1:assumed-role/AWSReservedSSO_x/dev@corp.com',
    }} enforcement={{ state: 'enforced_here', enforced: true }} />)
    // verified → the green Governed · IDC state + "enforcing" line
    expect(container.textContent).toMatch(/Governed · ◆ IDC/)
    expect(container.textContent).toMatch(/enforcing|active/i)
    expect(container.textContent).not.toMatch(/pending enforcement/i)
  })
})
...
describe('GovernanceApplyStatus reuse on UserDetail', () => {
  ...
  it('imports the shared component (does not re-define it)', () => {
    // Default import of the shared component (a named import of
    // applyLabel alongside it is fine — the same module is reused).
    expect(src).toMatch(
      /import\s+GovernanceApplyStatus(?:\s*,\s*\{[^}]*\})?\s+from\s+'\.\.\/components\/GovernanceApplyStatus'/)
...
// the Bedrock API key (IAM-user name) card — org-admin field
// that maps a developer to their mantle/Codex key for CUR attribution.
describe('BedrockKeySection', () => {
  const base = {
    identity_key: 'dev@corp.com', email: 'dev@corp.com', version: 3,
  }
  const noop = () => {}

  it('unmapped: shows em-dash + a "set key" affordance + the tg-never-sees-secret copy', () => {
    const { container } = render(
      <BedrockKeySection
        user={{ ...base, bedrock_key_user: null }}
        editing={false} draft="" setDraft={noop} busy={false}
        onEdit={noop} onCancel={noop} onSave={noop} />)
    expect(container.textContent).toMatch(/—/)
    expect(container.textContent).toMatch(/set key/i)
    expect(container.textContent).toMatch(/never sees the key secret/i)
  })

  it('mapped: shows the IAM-user name + an edit affordance', () => {
    const { container } = render(
      <BedrockKeySection
        user={{ ...base, bedrock_key_user: 'MantleApiKey-uhbhn79a' }}
        editing={false} draft="" setDraft={noop} busy={false}
        onEdit={noop} onCancel={noop} onSave={noop} />)
    expect(container.textContent).toMatch(/MantleApiKey-uhbhn79a/)
    expect(container.textContent).toMatch(/edit/i)
  })

  it('editing: Save fires onSave (the PATCH), Cancel fires onCancel', () => {
    const onSave = vi.fn()
    const onCancel = vi.fn()
    render(
      <BedrockKeySection
        user={{ ...base, bedrock_key_user: null }}
        editing={true} draft="MantleApiKey-new" setDraft={noop}
        busy={false} onEdit={noop} onCancel={onCancel} onSave={onSave} />)
...
// the api client contract — setBedrockKeyUser PATCHes the
// bedrock_key_user field with an If-Match version (the UI contract per
// the two-SPA-surface + test-the-api.js-contract rule).
describe('api.setBedrockKeyUser contract', () => {
  const src = readFileSync('src/api.js', 'utf8')
  it('PATCHes /api/users with bedrock_key_user + ifMatch version', () => {
    expect(src).toMatch(/setBedrockKeyUser:/)
    expect(src).toMatch(/bedrock_key_user: key/)
    expect(src).toMatch(/setBedrockKeyUser[\s\S]*ifMatch: ver/)
  })
})
