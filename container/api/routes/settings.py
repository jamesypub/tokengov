...
from db.invlogs_config import (
    get_invlogs_regions,
    set_invlogs_regions,
    get_invlogs_regions_updated_at,
)
...
# ── Bedrock invocation-logging (analytics capture) ──────────────────
# The region→bucket catalog + the Bedrock-API toggle. Separate from
# CUR spend: this stream carries per-invocation token/prompt data for a
# future model-selection recommender. Owner decision (slice 2): a
# region enable/disable calls Put/DeleteModelInvocationLoggingConfiguration
# against the install-provisioned bucket — no CFN-from-app — and only
# ENABLES when logging isn't already on (never clobbers the singleton).
...
def _account_id() -> str:
    ...
    return get_aws_session().client("sts").get_caller_identity()[
        "Account"]


def _bedrock_client(region: str):
    """A bedrock client bound to `region` (the config is a per-region
    singleton). Task-role cred chain; patched in tests."""
    from api.aws_session import get_aws_session
    return get_aws_session().client("bedrock", region_name=region)


@router.get("/settings/invocation-logs")
def get_invocation_logs_route(
    scope: Scope = Depends(_scope),
    db: Session = Depends(_db),
):
    """The invocation-logging region CATALOG — [{region, bucket,
    enabled, text_on}, …] — the 'where the analytics data lives' map a
    future analysis layer reads. org_admin scoped. updated_at powers
    the apply-status UI (pending vs applied)."""
    scope.require_org_admin()
    acct = ""
    try:
        acct = _account_id()
    except Exception:  # noqa: BLE001 — catalog still readable w/o acct
        acct = ""
    _updated = get_invlogs_regions_updated_at(db)
    return {
        "regions": get_invlogs_regions(db, acct),
        "updated_at": _updated.isoformat() if _updated else None,
    }


@router.put("/settings/invocation-logs")
def put_invocation_logs_route(
    body: dict,
    scope: Scope = Depends(_scope),
    db: Session = Depends(_db),
):
    """Replace the region catalog and APPLY it via the Bedrock API.
    Body: {"regions": [{"region": "us-east-1", "enabled": true,
    "text_on": true}, …]}. For each region: enabled → enable logging
    to that region's install-provisioned bucket (ONLY if logging isn't
    already on — never clobbers a pre-existing config); enabled:false
    → disable, but only when the live sink is OUR bucket. Returns the
    saved catalog + per-region apply results."""
    scope.require_org_admin()
    if "regions" not in body:
        raise HTTPException(400, "regions field required")
    try:
        acct = _account_id()
    except Exception as e:  # noqa: BLE001
        raise HTTPException(
            502, f"could not resolve the AWS account id: {e}")
    try:
        saved = set_invlogs_regions(db, body["regions"], acct)
    except ValueError as e:
        raise HTTPException(400, str(e))

    # Apply per region via the Bedrock API (each is a same-region
    # singleton). Failures are reported per-region, never fatal — the
    # catalog is saved regardless.
    ...
    for entry in saved:
        ...
        bucket = entry["bucket"]
        try:
            br = _bedrock_client(region)
            if entry["enabled"]:
                outcome = ia.enable_region(
                    br, bucket, text_on=entry["text_on"])
            else:
                outcome = ia.disable_region(br, bucket)
        except Exception as e:  # noqa: BLE001 — never fail the save
            outcome = ia.FAILED
            log = __import__("logging").getLogger("api.settings")
            log.warning("invlogs apply %s failed: %s", region, e)
        results.append({"region": region, "outcome": outcome})
    return {"regions": saved, "apply": results}
