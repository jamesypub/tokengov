...
def _provider_details(
    *,
    ...
    # Do NOT set RequestSigningAlgorithm for the AWS IDC SAML provider.
    # RequestSigningAlgorithm is a POOL-PROVIDER-LEVEL flag with no
    # per-message toggle: once set it signs BOTH the SLO LogoutRequest
    # AND the SP-initiated login AuthnRequest. AWS IAM Identity Center
    # (custom SAML app) accepts the signed logout but REJECTS the signed
    # AuthnRequest → "Federate 403 Forbidden" at the assertion endpoint,
    # so login never completes (no /auth/callback). Working login beats
    # clean logout, so signing stays OFF: the tradeoff is that
    # single-logout degrades (logout lands on the IDC screen instead of
    # the app /login). IDC does not validate SP request signatures for a
    # custom-SAML app, so SLO cannot be recovered by re-enabling this
    # flag — it would need a different mechanism.
    #
    # "Signing off" = the key is ABSENT. RequestSigningAlgorithm is an
    # enum whose ONLY member is "rsa-sha256" (per the Cognito SAML
    # signing docs), so real Cognito REJECTS "" with an
    # InvalidParameterException ("must satisfy enum value set:
    # [rsa-sha256]"). An earlier apply emitted "" to defeat the
    # UpdateIdentityProvider merge — but "" never reaches the provider,
    # it 400s the whole Save. So we OMIT the key here; clearing a stale
    # rsa-sha256 from a previously-signed provider is handled in
    # apply_saml_provider (delete+recreate — the only merge-safe,
    # Cognito-accepted clear), not by sending an invalid value.
    if metadata_url:
...
def apply_saml_provider(
    *,
    ...
    # Clearing a STALE rsa-sha256 is the one case a plain update can't
    # handle: UpdateIdentityProvider MERGES ProviderDetails (omitting the
    # key leaves the old value) and the enum rejects "" (there is no
    # "off" value to send). So when the existing provider still carries a
    # RequestSigningAlgorithm and we want it OFF (we never turn it on),
    # DELETE + RECREATE — a create starts from empty ProviderDetails, so
    # the recreated provider is unsigned. Common case (no stale value) is
    # a plain update. This is rare (only a provider signed by the old
    # buggy apply); the normal Save is an update or create.
    stale_signing = bool(
        (existing or {})
        .get("IdentityProvider", {})
        .get("ProviderDetails", {})
        .get("RequestSigningAlgorithm"))
    ...
    try:
        if existing is None:
            ...
            # Merge-safe clear: drop the signed provider, recreate unsigned.
