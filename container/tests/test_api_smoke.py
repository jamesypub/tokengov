...
def test_team_budget_persists_and_aggregates_spend(client):
    ...
    # 4. Aggregate spend pulls from members' CurUserSpend rows via the
    #    TeamMembership source of truth (the spend aggregator joins on
    #    TeamMembership, not the shadow User.team_id column). So place
    #    the member through assign_user_team() — the same helper the
    #    product uses — rather than writing the shadow column directly.
    usage_hour = datetime.now(timezone.utc).date()  # #643
    with get_db() as db:
        u = db.query(User).filter(
            User.email == "alice@test.com").first()
        if not u:
            db.add(User(email="alice@test.com", status="active"))
            db.flush()  # so assign_user_team's user lookup sees it
        # Membership is authoritative: creates the TeamMembership row
        # (and mirrors the shadow column) the aggregator joins against.
        assign_user_team(
            db, "alice@test.com", team_id, added_by="test")
        db.add(CurUserSpend(
