...
if [[ -z "${TG_VERSION:-}" ]]; then
  ...
  # the Docker build COPYs (container, admin-ui/web). Bare
  # `git status --porcelain` counted untracked scratch files (e.g.
  # internal/researcher2/) and flipped -dirty on a pristine tree.
...
# Cache-bust the compose SPA build on any admin-ui/web change — a
# stale ui-stage layer has served an old bundle (a "feature missing"
# that's a build artifact, not a code bug). The git tree hash of
# admin-ui/web is content-exact + fast; 'none' when not in a work tree.
UI_SRC_HASH=$(git rev-parse "HEAD:admin-ui/web" 2>/dev/null || echo none)
export UI_SRC_HASH
