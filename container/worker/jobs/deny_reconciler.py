...
# Per-principal synchronous reconcile states (returned to the API so a
# Govern/block/unblock/cap change reflects the ACTUAL post-apply
# outcome immediately — no ~5-min tick wait). Honest by construction:
# never claim ENFORCED unless the deny is verified reaching a role, and
# IDC stays PENDING (tg can't attach to an AWSReservedSSO_* role).
APPLY_ENFORCED = "enforced"          # deny applied + verified attached
APPLY_ALLOWED = "allowed"            # not denied (active) + verified
APPLY_PENDING = "pending"            # applied but not yet confirmed
APPLY_PENDING_IDC = "pending_idc"    # IDC — intent set, tg can't attach
APPLY_FAILED = "failed"              # the sync reconcile errored


def _principal_denied_now(db, user) -> bool:
    """Whether `user` SHOULD be denied right now, using the SAME pure
    predicate the reconciler tick uses (governed precondition + force-
    block OR over-cap on this month's billed spend). No IAM read — the
    intent the reconcile just applied."""
    default_cap = get_org_default_quota_usd(db)
    cap = user.cap_usd if user.cap_usd is not None else default_cap
    spend_row = (
        db.query(func.sum(CurUserSpend.spend_usd))
        .filter(
            CurUserSpend.email == user.email,
            CurUserSpend.usage_hour >= month_start_utc(),
        )
        .scalar()
    )
    spend = float(spend_row or 0.0)
    return bool(user.governed) and _should_deny(user, spend, cap)


def reconcile_principal(db, user) -> dict:
    """Apply the real IAM change for a single principal WITHIN
    the API request, instead of waiting for the ~5-min tick — then
    report the TRUTHFUL post-apply state.

    Single-writer invariant preserved: this does NOT attach/detach the
    deny itself. tg-BedrockQuotaDeny is one account-wide managed policy
    whose per-principal statements + role attachments are owned solely
    by `run()`, so a targeted change still means rebuilding that one
    policy — we invoke the SAME `run()` synchronously (one code path,
    no forked IAM logic) and then read back this principal's outcome.

    Returns {state, enforced, denied, detail}:
      state    — enforced / allowed / pending / pending_idc / failed
      enforced — True only for a tg-VERIFIED enforced deny (never on a
                 failed/unconfirmed apply; never for IDC).
      denied   — the intent the reconcile applied (should-deny now).
    The caller has already flipped + flushed status/governed; a failure
    here leaves that persisted and the periodic tick as the backstop."""
    denied = _principal_denied_now(db, user)

    # IDC: tg never AttachRolePolicy's an AWSReservedSSO_* role (the
    # IDC management-account boundary), so a synchronous
    # reconcile can't make it "enforced now" — the deny lands only via
    # the permission-set reference or tg-consumer, which tg can't
    # confirm here. Honest state = pending_idc (mirrors the
    # badge). The full reconcile still ran (below) to emit the
    # per-person statement + wire tg-consumer trust.
    is_idc = _is_idc(user)

    try:
        run()  # single-writer full reconcile, applied synchronously
    except Exception as e:  # noqa: BLE001 — status flip persists; tick backstop
        log.warning(
            "reconcile_principal: synchronous run() failed for %s "
            "(%s) — status persisted, periodic tick will retry",
            getattr(user, "email", "?"), e)
        return {
            "state": APPLY_FAILED, "enforced": False,
            "denied": denied,
    ...
    if is_idc:
        return {
            "state": APPLY_PENDING_IDC, "enforced": False,
            "denied": denied,
            "detail": "governance intent recorded; enforcement for an "
                      "Identity Center user activates once an identity "
                      "administrator applies the policy to their access",
        }

    # Non-IDC: verify the deny actually reaches the principal's role now
    # (the same read governance.verify uses). MANAGED = deny attached.
    role_name = _role_name_from_arn(getattr(user, "principal_arn", None))
    verified_attached = None
    if role_name:
        try:
            iam = boto3.client("iam", region_name=REGION)
            verified_attached = deny_attached_to_role(iam, role_name)
        except Exception:  # noqa: BLE001 — unverified → pending, not false-enforced
            verified_attached = None

    if denied:
        if verified_attached is True:
            return {"state": APPLY_ENFORCED, "enforced": True,
                    "denied": True, "detail": "deny enforced"}
        return {"state": APPLY_PENDING, "enforced": False,
                "denied": True,
                "detail": "deny applied; confirming attachment"}
    # Not denied → allowed. (Verify no stale deny lingers on the role.)
    if verified_attached is False or verified_attached is None:
        return {"state": APPLY_ALLOWED, "enforced": False,
                "denied": False, "detail": "active — not denied"}
    return {"state": APPLY_PENDING, "enforced": False, "denied": False,
            "detail": "clearing the deny; confirming"}


def deny_attached_to_role(iam, role_name: str) -> bool:
    """True if tg-BedrockQuotaDeny is attached to `role_name`
    (iam:ListAttachedRolePolicies). Raises on IAM failure (caller
    treats as unverified). One definition, reused by the sync
    per-principal readback."""
