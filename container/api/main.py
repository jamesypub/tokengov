...
async def lifespan(app: FastAPI):
    ...
    with engine.begin() as conn:
        ...
        # Admin-maintained email↔Bedrock-key mapping for CUR
        # attribution of bedrock-mantle / Codex (bearer-key) spend.
        # Stores the key's IAM-user NAME only (never the secret).
        # Nullable (most SSO-role users never need it); UNIQUE via a
        # separate index (ADD COLUMN can't inline UNIQUE idempotently)
        # so one key maps to at most one user — the fresh-install AND
        # the upgrade path both get the column + the uniqueness guard.
        conn.execute(text(
            "ALTER TABLE users "
            "ADD COLUMN IF NOT EXISTS bedrock_key_user VARCHAR"
        ))
        conn.execute(text(
            "CREATE UNIQUE INDEX IF NOT EXISTS "
            "ix_users_bedrock_key_user "
            "ON users (bedrock_key_user)"
        ))
    ...
    # TeamMembership becomes the single source of truth for a user's
    # team (one user, one team). Backfill a membership for every
    # existing user whose (shadow) team_id has no membership row, so
    # the members list/count/spend all resolve from the same source on
    # an upgraded install. Idempotent — creates only missing rows.
    from db.session import get_db as _gdb_mem
    from db.teams_membership import backfill_memberships
    with _gdb_mem() as db:
    ...
    # Seed the Bedrock invocation-logging region CATALOG from the
    # installer-provided TG_INVLOGS_REGIONS (comma-separated), like the
    # bootstrap admin. Insert-if-ABSENT only — never overwrites an
    # admin's later Settings edits on restart. This persists the
    # region->bucket map (what Settings + a future analysis layer read);
    # the per-region S3 stacks were already provisioned by the installer
    # and the Bedrock-API toggle applies on a Settings save / is
    # verified by the governance.invocation-logs doctor check.
    _invlogs_env = os.environ.get("TG_INVLOGS_REGIONS", "").strip()
    if _invlogs_env:
        from db.invlogs_config import (
            get_invlogs_regions, set_invlogs_regions)
        try:
            acct = os.environ.get("AWS_ACCOUNT_ID", "")
            with _gdb() as db:
                if not get_invlogs_regions(db, acct):
                    entries = [
                        {"region": r.strip(), "enabled": True,
                         "text_on": True}
                        for r in _invlogs_env.split(",") if r.strip()
                    ]
                    set_invlogs_regions(db, entries, acct)
        except Exception:  # noqa: BLE001 — seed is best-effort, never blocks boot
            pass
    yield
