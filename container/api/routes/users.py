...
# How long the synchronous per-principal reconcile may run
# before we degrade to a truthful "pending" (the periodic tick remains
# the backstop) rather than hang the request. Env-tunable.
_SYNC_RECONCILE_TIMEOUT_S = float(
    os.environ.get("TG_SYNC_RECONCILE_TIMEOUT_S", "5"))


def _apply_now(db: Session, u: User) -> dict:
    """Apply the real IAM change for `u` WITHIN this request via
    the reconciler's shared per-principal path (single writer), bounded
    by a timeout. Returns the truthful apply state
    (deny_reconciler.reconcile_principal) or a 'pending' fallback on
    timeout/error — NEVER a false 'enforced'. The caller has already
    flipped + flushed status/governed, so a fallback still persists the
    intent and the periodic tick applies it.

    Runs in a worker thread with a timeout: the reconcile does blocking
    IAM I/O, and a wedged/throttled call must degrade rather than hang
    the API."""
    from concurrent.futures import ThreadPoolExecutor, TimeoutError \
        as _FTimeout
    from worker.jobs import deny_reconciler as _dr
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            fut = ex.submit(_dr.reconcile_principal, db, u)
            return fut.result(timeout=_SYNC_RECONCILE_TIMEOUT_S)
    except _FTimeout:
        return {
            "state": _dr.APPLY_PENDING, "enforced": False,
            "detail": "still applying; the background reconciler will "
                      "finish shortly",
        }
    except Exception as e:  # noqa: BLE001 — never fail the action on apply
        return {
            "state": _dr.APPLY_FAILED, "enforced": False,
            "detail": f"apply not confirmed ({e}); the background "
...
def _add_idc_consumer_trust(principal_arn: str | None) -> None:
    ...
    try:
        # Build the client INSIDE the guard: a session/client
        # construction failure (ProfileNotFound / NoCredentials, a
        # BotoCoreError) is not a ClientError and would otherwise 500.
        # Govern can't wire the trust without a working client, so
        # surface the same truthful 502 (never a bare 500).
        iam = _iam_client()
        ...
        raise HTTPException(
            502,
            f"could not reach IAM to add the IDC trust to {role} "
            f"({e}). Govern did NOT complete.",
        )
...
def _remove_idc_consumer_trust_if_unused(db: Session, u: User) -> None:
    ...
    from botocore.exceptions import ClientError, BotoCoreError
    try:
        # Build the client inside the guard (a construction failure is
        # not a ClientError and would 500 — see _add_idc_consumer_trust).
        iam = _iam_client()
        cur = iam.get_role(RoleName=role)
        doc = (cur.get("Role") or {}).get(
            "AssumeRolePolicyDocument") or {}
        ...
        raise HTTPException(
            502,
            f"could not reach IAM to remove the IDC trust from {role} "
            f"({e}).",
        )
...
def _user_dict(
    ...
    return {
        ...
        # the IAM-user name of this user's Bedrock API key, for
        # mantle/Codex CUR attribution (null until an admin maps it).
        "bedrock_key_user":  u.bedrock_key_user,
...
def _consumer_trust_wired(iam, principal_arn: str | None) -> bool:
    """True if this IDC user's SSO permission-set role is trusted to
    assume tg-consumer (tg wires this on Govern). Read-only
    (iam:GetRole on tg-consumer — already granted). Any read failure →
    False (treated as not-confirmed, never a false 'enforced')."""
    from api import idc_trust
    arnlike = idc_trust.permset_arnlike(principal_arn)
    if not arnlike:
        return False
    try:
        cur = iam.get_role(RoleName=_consumer_role_name())
        doc = (cur.get("Role") or {}).get(
            "AssumeRolePolicyDocument") or {}
        return idc_trust.has_trust(doc, arnlike)
    except Exception:  # noqa: BLE001 — best-effort read; unread = False
        return False


def _deny_attached_to_role(iam, role_name: str) -> bool | None:
    """True/False whether tg-BedrockQuotaDeny is attached to `role_name`
    (iam:ListAttachedRolePolicies — already granted). None on a read
    failure so the classifier can report UNKNOWN rather than a false
    negative."""
    import governance
    try:
        return governance.deny_attached(iam, role_name)
    except Exception:  # noqa: BLE001 — unreadable → UNKNOWN, not False
        return None


def _deny_attached_to_consumer(iam) -> bool | None:
    """True/False whether tg-BedrockQuotaDeny is attached to
    tg-consumer, via iam:ListEntitiesForPolicy on the deny policy
    (already granted, Sid ListQuotaDenyAttachments). None on failure →
    UNKNOWN."""
    consumer = _consumer_role_name()
    try:
...
@router.get("/users/{email}/idc-enforcement")
def idc_enforcement(
    email: str,
    scope: Scope = Depends(_scope),
    db: Session = Depends(_db),
):
    """For a *governed* IDC (`AWSReservedSSO_*`) user, report
    whether the deny is ACTUALLY enforced — a state tg can verify with
    read-only IAM in its own member account — so the UI never presents
    unverified intent as enforced.

    Returns {state, enforced} where state is one of enforced_here /
    enforced_via_consumer / pending / unknown (idc_enforcement.classify).
    Only meaningful for a governed IDC user; a non-IDC or ungoverned
    user returns state='pending' with enforced=False (the UI only calls
    this for the governed-IDC badge)."""
    from api import idc_enforcement as ie
    u = _resolve_user_or_404(db, email)
    # Same read authz as get_user: org admin any / team admin subtree /
    # self.
    if (not scope.is_org_admin
            and u.team_id not in scope.admin_team_ids
            and not scope.is_self(u)):
    ...
    if (u.role_type or "iam") != "idc" or not u.governed:
        return {"state": ie.PENDING, "enforced": False}

    # Build the client + run the reads inside one guard. The per-read
    # helpers already degrade a CALL failure to None/False, but building
    # the client/session can itself fail (ProfileNotFound, NoCredentials,
    # any botocore config error) — that must degrade to UNKNOWN too, not
    # 500. Honors the module contract ("a failed read → unknown, never a
    # false enforced") and the no-500-to-the-browser rule.
    try:
        iam = _iam_client()
        sso_role = ie.idc_role_name(u.principal_arn)
        sso_attached = (
            _deny_attached_to_role(iam, sso_role) if sso_role else False)
        consumer_attached = _deny_attached_to_consumer(iam)
        trust_wired = _consumer_trust_wired(iam, u.principal_arn)
    except Exception:  # noqa: BLE001 — client/session build failed → UNKNOWN
        return {"state": ie.UNKNOWN, "enforced": False}
    state = ie.classify(
        sso_role_attached=sso_attached,
        consumer_attached=consumer_attached,
        consumer_trust_wired=trust_wired,
    )
    return {"state": state, "enforced": ie.is_enforced(state)}
...
def patch_user(
    ...
    if not u:
        raise HTTPException(404, f"User {email} not found")
    # Two mutable fields, DIFFERENT authz:
    #   display_name — self-service: the user themselves, OR an admin
    #     per the 3-tier rule (org_admin / team_admin of the user's
    #     team).
    #   bedrock_key_user — org-admin ONLY (it drives fleet-wide
    #     CUR spend ATTRIBUTION, not a cosmetic label; a mis-set value
    #     mis-attributes another person's mantle/Codex spend).
    # Reject any other key so callers can't smuggle a caller/principal
    # edit through this endpoint.
    allowed = {"display_name", "bedrock_key_user"}
    extra = set(body.keys()) - allowed
    if extra:
        raise HTTPException(
            400,
            f"PATCH /users only updates {sorted(allowed)}; "
            f"unexpected field(s): {sorted(extra)}",
        )
    if not (body.keys() & allowed):
        raise HTTPException(
            400,
            "one of display_name / bedrock_key_user is required")
    # Authorize per the fields actually present: display_name needs
    # self-or-team-admin; bedrock_key_user needs org_admin. Check the
    # stricter one that applies (org_admin implies self-or-team-admin).
    if "bedrock_key_user" in body:
    ...
    if "bedrock_key_user" in body:
        # The IAM-user NAME of the user's long-term Bedrock API key
        # (never the secret). Accept a bare name OR a full ARN /
        # `user/<name>` and normalize to the name (the CUR join key is
        # the name). null / "" clears the mapping.
        key = body.get("bedrock_key_user")
        if key is not None:
            if not isinstance(key, str):
                raise HTTPException(
                    400, "bedrock_key_user must be a string or null")
            key = key.strip()
            if ":user/" in key:
                key = key.split(":user/", 1)[1]
            elif key.startswith("user/"):
                key = key[len("user/"):]
            key = key or None
        if key is not None:
            # Uniqueness: one key maps to at most one user. A value
            # already mapped to ANOTHER user is a 409 (would
            # mis-attribute that key's spend). Re-setting the SAME
            # user's own value is fine (idempotent).
            clash = (
                db.query(User)
                .filter(
                    User.bedrock_key_user == key,
                    User.email != u.email,
                )
                .first()
            )
            if clash is not None:
                raise HTTPException(
                    409,
                    f"bedrock_key_user '{key}' is already mapped to "
                    f"{clash.email}",
                )
        u.bedrock_key_user = key
...
def manage_principal(
    ...
    **deny-only** — tg holds no grant power; worst case it
    ...
    if (u.role_type or "iam") == "idc":
        ...
        # Emit the per-person statement + wire trust now (the
        # reconcile still runs); IDC state is truthfully pending — tg
        # can't attach to an AWSReservedSSO_* role (honest pending).
        out["apply"] = _apply_now(db, u)
        return out
    ...
    from botocore.exceptions import ClientError, BotoCoreError
    try:
        # Build the client + resolve the policy ARN INSIDE the guard: a
        # session/client construction failure (ProfileNotFound /
        # NoCredentials) is a BotoCoreError, not a ClientError, and
        # would otherwise 500. Govern can't attach the deny without a
        # working client → truthful 502, never a bare 500.
        iam = _iam_client()
        policy_arn = _deny_policy_arn(iam)
        ...
        raise HTTPException(
            502,
            f"could not reach IAM to attach the deny to {role_name} "
            f"({e}). Govern did NOT complete.",
        )
    ...
    spend = _user_spend(db, ident)
    out = _user_dict(u, spend, db=db, models=models)
    # Apply the deny NOW (synchronous reconcile) — return the
    # real enforced/allowed state, not "pending ~5 min".
    out["apply"] = _apply_now(db, u)
    return out
...
def unmanage_principal(
    ...
    if role_name and (u.role_type or "iam") != "idc":
        ...
        if others == 0:
            from botocore.exceptions import ClientError, BotoCoreError
            try:
                # Build the client + resolve the ARN inside the guard: a
                # construction failure is a BotoCoreError, not a
                # ClientError, and would otherwise 500 (mirror the
                # manage path above).
                iam = _iam_client()
                policy_arn = _deny_policy_arn(iam)
                ...
                raise HTTPException(
                    502,
                    f"could not reach IAM to detach the deny from "
                    f"{role_name} ({e}). Ungovern did NOT complete.",
                )
...
def set_cap(
    ...
    # A cap change that crosses the threshold should re-evaluate
    # enforcement NOW, not on the next tick. Only governed principals
    # are enforced, so skip the (whole-account) reconcile for the
    # unmanaged case — nothing to apply.
    if u.governed:
        out["apply"] = _apply_now(db, u)
    return out
...
def create_user(
    ...
    db.add(u)
    db.flush()  # the users row must exist before the membership FK
    # Team is set through the membership source of truth (one user, one
    # team) — never as an independent column write. A brand-new user is
    # in no team, so this just creates the membership when a team is
    # given (it also shadows u.team_id).
    from db.teams_membership import assign_user_team
    assign_user_team(db, email, body.get("team_id"), added_by=scope.email)
    db.flush()
    return _user_dict(u, db=db)
...
def set_team(
    ...
    # Team is set via the membership source of truth (one user, one
    # team): same team → no-op, clear → remove, a DIFFERENT team while
    # already in one → 409 (remove them from the other team first).
    # assign_user_team also shadows u.team_id so reads stay consistent.
    from db.teams_membership import assign_user_team
    assign_user_team(db, u.email, team_id, added_by=scope.email)
    u.version = (u.version or 1) + 1
    db.flush()
    return _user_dict(u, db=db)
...
def force_block_user(
    ...
    u.version = (u.version or 1) + 1
    db.flush()
    # Apply the deny NOW (synchronous reconcile), not on the
    # ~5-min tick — return the truthful post-apply state.
    out = _user_dict(u, db=db)
    out["apply"] = _apply_now(db, u)
    return out
...
def unblock_user(
    ...
    # not-force-blocked resting state — the reconciler re-evaluates
    # spend vs cap and may immediately re-deny if still over cap.
    u.status = "active"
    u.force_blocked_at = None
    u.version = (u.version or 1) + 1
    db.flush()
    # Run the reconcile NOW and return the TRUTHFUL outcome —
    # either allowed (active), or still denied if over cap (no cap-free
    # window). apply.denied tells the UI which, so it never claims
    # "unblocked = allowed" when the reconcile re-denied.
    out = _user_dict(u, db=db)
    out["apply"] = _apply_now(db, u)
    return out
